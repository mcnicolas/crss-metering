...
public class MDEFParser {
    ...
    public Header readMtMeterHeader(byte[] record) {
        ...
        header.setrLen(transformByte(0, 1, 'i', record));
        header.setrCode(transformByte(2, 3, 'i', record));
        header.setCmCustid(transformByte(4, 23, 'c', record));
        header.setCmName(transformByte(24, 43, 'c', record));
        header.setCmAddr1(transformByte(44, 63, 'c', record));
        header.setCmAddr2(transformByte(64, 83, 'c', record));
        header.setCmAccount(transformByte(84, 103, 'c', record));
        header.setCmLogchans(transformByte(111, 114, 'c', record));
        header.setTaStart(transformByte(119, 130, 'c', record));
        header.setTaStop(transformByte(131, 142, 'c', record));
        header.setDstFlag(transformByte(143, 143, 'c', record));
        return header;
    }
    ...
    public ChannelHeader readMtChannelHeader(byte[] record) {
        ...
        header.setrLen(transformByte(0, 1, 'i', record));
        header.setrCode(transformByte(2, 3, 'i', record));
        header.setDcCustid(transformByte(4, 23, 'c', record));
        header.setDcRecid(transformByte(24, 37, 'c', record));
        header.setDcMeterid(transformByte(44, 55, 'c', record));
        header.setTaStart(transformByte(56, 67, 'c', record));
        header.setTaStop(transformByte(68, 79, 'c', record));
        header.setDcPyschan(transformByte(93, 94, 'c', record));
        header.setDcLogchan(transformByte(95, 96, 'i', record));
        header.setDcUmcode(transformByte(97, 98, 'c', record));
        header.setChanStat(transformByte(99, 99, 'c', record));
        header.setIntstat(transformByte(100, 100, 'c', record));
        header.setStrtmtr(transformByte(101, 112, 'c', record));
        header.setStopmtr(transformByte(113, 124, 'c', record));
        header.setDcMmult(transformByte(126, 135, 'c', record));
...
//				header.setDcServerType(dcServerType);
//			}
//		}

        header.setDcServerType(transformByte(166, 166, 'c', record));
        header.setDrInphr(transformByte(177, 178, 'c', record));
        header.setTdStatus(transformByte(193, 194, 'c', record));
        header.setDcFlow(transformByte(210, 210, 'c', record));
        header.setDcKvaset(transformByte(211, 212, 'i', record));
        header.setTdOrigin(transformByte(213, 213, 'c', record));
        ...
        try {
            ...
            while (data.read(b, 0, recordSize) >= 0) {
                ...
                if (dataRcode.equals("1")) {
                    ...
                    header = readMtMeterHeader(b);
                    mdef.getHeaders().add(header);
                    ...
                    if (header != null) {
                        header.getChannels().add(channelHeader);
                    }
